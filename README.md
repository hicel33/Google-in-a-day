...
Indexed pages are appended to **`data/storage/index.csv`** (CSV with a header row). That folder is **wiped when the API process starts** and again at each **Start crawl**, so nothing carries over between backend restarts. Override the directory with the **`GOOGLE_IN_A_DAY_STORAGE`** environment variable if needed.
