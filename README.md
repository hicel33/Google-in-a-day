...
Indexed pages are appended to **`data/storage.csv`** (CSV with a header row). That file is **wiped when the API process starts** and again at each **Start crawl**, so nothing carries over between backend restarts. Override the path with the **`GOOGLE_IN_A_DAY_STORAGE`** environment variable if needed.
